...
const Transcript=({transcript,engTranscript})=>{
 ...
 return (<div className="Box" title="Transcript of the youtube video">
   ...
   <nav>
    <div>
      <button className="navButton" onClick={()=>setDisplayTranscript(true)}>Orignal transcript</button>
      </div>
      <Image src="/youtube.svg" height={50} width={50} alt="youtube svg"/>
      <div>
      <button className="navButton" onClick={()=>setDisplayTranscript(false)}>English transcript</button>
      </div>
   </nav>
  <div className="co">
     <div className="coin">
      {displayTranscript?transcript:engTranscript}
     </div>
  </div>
  <nav>
